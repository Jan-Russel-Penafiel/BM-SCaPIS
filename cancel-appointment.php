...
try {
    ...
    if (!$appointment) {
        throw new Exception('Appointment not found or not eligible for cancellation.');
    }
    ...
    ");
    $stmt->execute([
        date('Y-m-d H:i:s') . ' - ' . ucfirst($appointment['appointment_type']) . ' appointment cancelled by ' . $_SESSION['first_name'] . ' ' . $_SESSION['last_name'],
    ...
    $stmt->execute([
        $appointment['application_id'],
        ucfirst($appointment['appointment_type']) . ' appointment cancelled',
        $_SESSION['user_id']
    ]);
    ...
    if ($appointment['sms_notifications'] && $appointment['contact_number']) {
        ...
        $message = "Hi {$appointment['first_name']}, your {$appointmentTypeText} appointment for {$appointment['type_name']} application #{$appointment['application_number']} scheduled for " . date('M j, Y g:i A', strtotime($appointment['appointment_date'])) . " has been cancelled. Please contact the barangay office for rescheduling.";
        ...
        if (!$sms_result['success']) {
            error_log('SMS notification failed for appointment cancellation: ' . $sms_result['message']);
        }
    }
    ...
    if ($appointment['email_notifications'] && $appointment['email']) {
        $appointmentTypeText = ucfirst($appointment['appointment_type']);
        $subject = "{$appointmentTypeText} Appointment Cancelled - Application #{$appointment['application_number']}";
        $message = "
        Dear {$appointment['first_name']} {$appointment['last_name']},
        
        Your {$appointmentTypeText} appointment for the following application has been cancelled:
        
        Application Details:
        - Application Number: {$appointment['application_number']}
        - Document Type: {$appointment['type_name']}
        - Appointment Type: {$appointmentTypeText}
        - Original Appointment Date: " . date('F j, Y g:i A', strtotime($appointment['appointment_date'])) . "
        
        Please contact the barangay office to reschedule your appointment if needed.
    ...
    logActivity(
        $_SESSION['user_id'],
        'Cancelled ' . $appointment['appointment_type'] . ' appointment for application #' . $appointment['application_number'],
        'appointments',
        $appointmentId
    );
    ...
    $_SESSION['success'] = ucfirst($appointment['appointment_type']) . ' appointment cancelled successfully.';
    header('Location: appointments.php');
    exit;
